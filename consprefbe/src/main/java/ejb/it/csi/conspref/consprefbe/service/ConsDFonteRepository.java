...
@Stateless
public class ConsDFonteRepository {
	    ...
	    public ConsDFonte ricercaFonteByCodice(String codice)  {
	    	final String METHOD_NAME = "ricercaFonteByCodice";
			log.debug(METHOD_NAME, "get service for: codice=%s", codice);

	    	StringBuilder sb = new StringBuilder();
	    	sb.append("SELECT c FROM ConsDFonte c ");
	    	...
	    	try {
	    		...
	    		String msg = (String)properties.getProp().get(E_ERRORI.R_CODICE_FONTE_WRONG.getErrorCode()) + " - " + codice;
	    		log.error(METHOD_NAME, msg, nre);
	    		throw new BeServiceException(E_ERRORI.R_CODICE_FONTE_WRONG.getErrorCode(), msg);
	    	}
	    	log.debug(METHOD_NAME, "result cod asr: %s", result.getFonteCod());
	    	return result;
