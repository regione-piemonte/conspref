...
@Stateless
public class ConsDSottoTipoConsensoRepository {
	    ...
	    public ConsDSottoTipoCon ricercaSottoTipoConsensoByCodice(String codiceEnum)  {
	    	final String METHOD_NAME = "ricercaSottoTipoConsensoByCodice";
			log.debug(METHOD_NAME, "get service for: codiceEnum=%s", codiceEnum);

	    	StringBuilder sb = new StringBuilder();
	    	sb.append("SELECT c FROM ConsDSottoTipoCon c ");
	    	sb.append("WHERE c.dataScadenza IS NULL ");
	    	
	    	Map<String, Object> params = new HashMap<>();
	    	populateFilter(sb, params, codiceEnum);
	    	
	    	String jpql = sb.toString();
	    	log.debug(METHOD_NAME, "jpql: %s", jpql);
	    	TypedQuery<ConsDSottoTipoCon> query = em.createQuery(jpql, ConsDSottoTipoCon.class);
	    	...
	    	try {
	    		...
	    		String msg = (String)properties.getProp().get(E_ERRORI.R_CODICE_SOTTOTIPO_CONSENSO_WRONG.getErrorCode()) + " - " + codiceEnum;
	    		log.error(METHOD_NAME, msg, nre);
	    		throw new BeServiceException(E_ERRORI.R_CODICE_SOTTOTIPO_CONSENSO_WRONG.getErrorCode(), msg);
	    	}
	    	log.debug(METHOD_NAME, "result delestatoId: %s", result.getSottoTipoConsenso());
	    	return result;
	    	
	    }
	    
		private void populateFilter(StringBuilder sb, Map<String, Object> params, String codice) {
			if(StringUtils.isNotBlank(codice)) {
				sb.append("AND c.sottoTipoConsenso = :codice ");
