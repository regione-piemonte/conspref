...
@Stateless
public class ConsDTipoConsensoRepository {
	    ...
	    public enum ConsDTipoConsensoEnum {
	    	A,
	    	R
	    }
...
/
	    ...
	    public ConsDTipoCon ricercaTipoConsensoByCodice(String codiceEnum)  {
	    	...
	    	TypedQuery<ConsDTipoCon> query = em.createQuery(jpql, ConsDTipoCon.class);
	    	...
	    	try {
	    		...
	    		String msg = (String)properties.getProp().get(E_ERRORI.R_CODICE_TIPO_CONSENSO_WRONG.getErrorCode()) + " - " + codiceEnum;
	    		log.error(METHOD_NAME, msg, nre);
	    		throw new BeServiceException(E_ERRORI.R_CODICE_TIPO_CONSENSO_WRONG.getErrorCode(), msg);
	    	}
	    	log.debug(METHOD_NAME, "result delestatoId: %s", result.getTipoConsenso());
	    	return result;
		...
		private void populateFilter(StringBuilder sb, Map<String, Object> params, String codice) {
			if(StringUtils.isNotBlank(codice)) {
				sb.append("AND c.tipoConsenso = :codice ");
