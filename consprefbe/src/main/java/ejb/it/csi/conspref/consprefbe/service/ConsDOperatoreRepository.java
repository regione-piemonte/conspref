...
@Stateless
public class ConsDOperatoreRepository {
	    ...
	    public ConsDOperatore ricercaOperatoreByCodice(String codiceEnum)  {
	    	final String METHOD_NAME = "ricercaOperatoreByCodice";
			log.debug(METHOD_NAME, "get service for: codiceEnum=%s", codiceEnum);

	    	StringBuilder sb = new StringBuilder();
	    	sb.append("SELECT c FROM ConsDOperatore c ");
	    	...
	    	TypedQuery<ConsDOperatore> query = em.createQuery(jpql, ConsDOperatore.class);
	    	...
	    	try {
	    		...
	    		String msg = (String)properties.getProp().get(E_ERRORI.R_CODICE_OPERATORE_WRONG.getErrorCode()) + " - " + codiceEnum;
	    		log.error(METHOD_NAME, msg, nre);
	    		throw new BeServiceException(E_ERRORI.R_CODICE_OPERATORE_WRONG.getErrorCode(), msg);
	    	}
	    	log.debug(METHOD_NAME, "result delestatoId: %s", result.getCodOperatore());
	    	return result;
	    	
	    }
	    
		
		private void populateFilter(StringBuilder sb, Map<String, Object> params, String codOperatore) {
			if(StringUtils.isNotBlank(codOperatore)) {
				sb.append("AND c.codOperatore = :codOperatore ");
				params.put("codOperatore", codOperatore);
			}
		}
}
