...
public class RevocaConsensoService extends BaseService<RevocaConsensoRichiesta, RevocaConsensoRicevuta> {
	...
	@Override
	protected void checkServiceParams(RevocaConsensoRichiesta req) {
		...
		if(req.getElencoAsr().isEmpty()) {
			throw new ServiceException(E_ERRORI.R_CODICE_ASR_MISSING.getErrorCode(), (String)properties.getProp().get(E_ERRORI.R_CODICE_ASR_MISSING.getErrorCode()), RisultatoCodice.BLOCCANTE);
		}
	...
	@Override
	protected RevocaConsensoRicevuta execute(RevocaConsensoRichiesta req) throws ParseException {
		
		CsiLogAudit logaudit = saveLogAudit(req);
		
		RevocaConsensoRicevuta res = new RevocaConsensoRicevuta();
		...
		if(response.getBody()!=null && response.getBody().getElencoProfili()!=null 
			...
			consensoBean.revocaConsensoRegione(req, logaudit);
	...
	private CsiLogAudit saveLogAudit(RevocaConsensoRichiesta req) {
