...
rem DTO LATO SERVER
java -cp %CLI_JAR_PATH%;%CUSTOM_GEN_JAR_PATH% %DEBUG_OPTS% io.swagger.codegen.SwaggerCodegen generate -i ..\src\yaml\consprefboweb.yaml -l jaxrs-resteasy-eap-csi -o .. --config swagger_config_java.json

rem DTO LATO CLIENT
remjava -cp %CLI_JAR_PATH%;%CUSTOM_GEN_JAR_PATH% %DEBUG_OPTS% io.swagger.codegen.SwaggerCodegen generate -i ..\src\yaml\consprefboweb.yaml -l typescript-angular -o ..\docs\yaml\cli-original --config swagger_config_java.json
...
java -jar %CLI_JAR_PATH% generate -l html -i ..\src\yaml\consprefboweb.yaml -o ..\docs\yaml